...
static void send_fig_fm260b_cmd(uint8_t *p_data, uint16_t length)
{
	...
	fig_param_second = nus_data_array[5] *256 + nus_data_array[6];
	//将获取的指令发送给指纹模块
	...
	if(nus_data_array[2] == 0x20)//自动注册模块指令
	{
		//设置自动注册状态位
		is_autoenroll = true;
	}
...
void operate_code_check(uint8_t *p_data, uint16_t length)
{
	...
	switch(p_data[0])
	{
		...
		if(length ==0x0a)//10字节
		{
			cmd_set_key(nus_data_array, nus_data_array_length);
		}
		break;
		...
		if(length ==0x08)//8字节
		{
			sync_rtc_time(nus_data_array,nus_data_array_length);
		}
		break;
		
		case GET_TIME://获取时间
			get_rtc_time(nus_data_array, nus_data_array_length);
		break;
		
		case GET_KEY_NOW://获取现在的动态密码
			get_key_now(nus_data_array, nus_data_array_length);
		break;
		
		case SET_PARAMS://设置参量
		if(length == 0x7)//6字节
		{
			set_param(nus_data_array, nus_data_array_length);
		}
		break;
		...
		if(length == 0x11)//17字节
		{
			set_key_seed(nus_data_array, nus_data_array_length);
		}
		break;
		...
		if(length ==0x07)//7字节
		{
			set_mac(nus_data_array, nus_data_array_length);
		}
		break;
		...
		if(length == 0x0d)//13字节
		{
			set_super_key(nus_data_array, nus_data_array_length);
		}
		break;
		
		case GET_USED_KEY://查询有效密码
			get_used_key(nus_data_array, nus_data_array_length);
		break;
		
		case GET_RECORD_NUMBER://查询开门记录数量
			get_record_number(nus_data_array, nus_data_array_length);
		break;
		
		case GET_RECENT_RECORD://查询指定日期后的记录
		if(length == 0x08)//8字节
		{
			get_recent_record(nus_data_array, nus_data_array_length);
		}
		break;
