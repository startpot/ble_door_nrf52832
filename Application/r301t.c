		...
		if(fig_send_data_array_length ==(9+reply_data_len))
		{
			...
			if(is_r301t_autoenroll ==false)
			{
				if(	r301t_autosearch_step >0)
				{
					...
					{
						if(	r301t_autosearch_step == 1 &&fig_send_data_array_length != 0)
						{
						...
						if( r301t_autosearch_step == 2 &&fig_send_data_array_length != 0)
						{
						...
						if( r301t_autosearch_step ==3 &&fig_send_data_array_length != 0)
						{
							//最后一步，判断结果
							if(fig_send_data_array[9] == 0)
							{	//返回搜索到了指纹
							ble_door_open();
							}
							//设置步骤为0，状态为false
